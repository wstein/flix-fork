...
object Eraser {
  ...
  private def visitType(tpe0: SimpleType)(implicit ctx: SharedContext, flix: Flix): SimpleType = {
    ...
    tpe0 match {
      ...
      // The type arguments are what the boundary declares, so they are visited rather than
      // erased away; the representation is `clazz` alone and is unaffected either way.
      case Native(clazz, targs) => Native(clazz, targs.map(visitType))
    }
  }
  ...
  private final class SharedContext {
    ...
    def getSpecializedEnumName(sym: Symbol.EnumSym, targs: List[SimpleType])(implicit flix: Flix): Symbol.EnumSym = {
      targs match {
        ...
        case _ =>
          // Do specialization.
          enumSpecializations.computeIfAbsent((sym, targs), _ => Symbol.specializedEnumSym(sym, targs))
      }
    }
