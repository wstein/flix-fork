...
/**
  * A sequence of layout pieces rendered contiguously without additional split choices.
  */
final case class AdjacentPiece(children: List[Piece]) extends Piece {

  override def format(writer: CodeWriter, state: State): Unit = {
    children.foreach(child => writer.format(child))
  }
