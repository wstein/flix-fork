...
class TestFlixPackageManagerInstall extends AnyFunSuite {
  ...
  test("install succeeds from the first candidate that hits, without reading the listing") {
    ...
    val scripts = List(
      ...
      Map(assetUrl("museum.fpkg") -> List(CannedResponse(404)), assetUrl("museum-clerk.fpkg") -> List(CannedResponse(200, body = "package-bytes")))
    )
    for (script <- scripts) {
      implicit val transport: GitHub.Transport = FakeTransport(script)
      val root = Files.createTempDirectory("install-test")
      FlixPackageManager.install("flix/museum", version, "fpkg", List("museum.fpkg", "museum-clerk.fpkg"), root, apiKey = None) match {
  ...
  test("install falls back to the listing when every candidate 404s") {
    implicit val transport: GitHub.Transport = FakeTransport(Map(
      ...
      tagUrl -> List(CannedResponse(200, body =
        """{"tag_name":"v1.0.0","assets":[{"name":"published-under-a-different-name.fpkg","browser_download_url":"https://example.invalid/asset.fpkg"}]}""")),
  ...
  test("a download that fails partway through is not cached as a successful install") {
    implicit val transport: GitHub.Transport = FakeTransport(Map(
      assetUrl("museum.fpkg") -> List(CannedResponse(200, body = "a-lot-of-package-bytes", failAfterBytes = Some(4)))
    ))
    val root = Files.createTempDirectory("install-test")

    val result = FlixPackageManager.install("flix/museum", version, "fpkg", List("museum.fpkg"), root, apiKey = None)
    assert(result.toOption.isEmpty, s"expected the failed download to be reported as an error, got $result")

    val dirPath = Bootstrap.getLibraryDirectory(root).resolve("github").resolve("flix").resolve("museum").resolve(version.toString)
    val filesLeftBehind = Files.list(dirPath).iterator().asScala.toList
