...
/**
  ...
  */
case class CannedResponse(status: Int, headers: Map[String, String] = Map.empty, body: String = "", failAfterBytes: Option[Int] = None)
...
object FakeTransport {
  ...
  private def toHttpResponse(req: HttpRequest, canned: CannedResponse): HttpResponse[InputStream] = new HttpResponse[InputStream] {
    ...
    override def headers(): HttpHeaders =
      HttpHeaders.of(canned.headers.view.mapValues(v => java.util.List.of(v)).toMap.asJava, (_, _) => true)
