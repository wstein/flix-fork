...
/**
  ...
  * is written down.
  */
class TestCliContract extends AnyFunSuite {
  ...
  test("a client that states no version is served") {
    ...
    val (compatible, document) = CliContract.describe(None)
    assert(compatible)
    assertResult(Some(JBool(true)))(field(document, "success"))
  }

  test("a client speaking the current version is served") {
    val (compatible, _) = CliContract.describe(Some(CliContract.ProtocolVersion))
    assert(compatible)
  }
  ...
  test("a client from the future is refused, by number and in words") {
    ...
    val (compatible, document) = CliContract.describe(Some(CliContract.ProtocolVersion + 1))
    assert(!compatible)
    assertResult(Some(JBool(false)))(field(document, "success"))
    val error = field(document, "error").collect { case JString(s) => s }.getOrElse("")
    assert(error.contains((CliContract.ProtocolVersion + 1).toString), error)
  }

  test("a client older than we still serve is refused") {
    val (compatible, _) = CliContract.describe(Some(CliContract.MinimumClientVersion - 1))
    assert(!compatible)
  }
  ...
  test("a refusal still says what this compiler does speak") {
    ...
    val (_, document) = CliContract.describe(Some(CliContract.ProtocolVersion + 1))
    assertResult(Some(JInt(CliContract.ProtocolVersion)))(field(document, "protocolVersion"))
    assertResult(Some(JInt(CliContract.MinimumClientVersion)))(field(document, "minimumClientVersion"))
  }
